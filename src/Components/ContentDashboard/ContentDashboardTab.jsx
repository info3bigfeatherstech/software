...
export default function ContentDashboardTab() {
    ...
    const locationLabel = isAdmin()
        ? "All Locations"
        : `${CURRENT_USER.locationId.startsWith("WH") ? "Warehouse" : "Shop"}: ${CURRENT_USER.locationId}`;
    ...
    return (
        <div className="space-y-6">
            ...
            {/* ── Secondary KPIs ───────────────────────────────────────────── */}
            <div className="grid grid-cols-4 gap-4">
                <StatCard label="Products in Scope" value={scopedProducts.length} sub={isAdmin() ? `across ${shops.length} locations` : `at ${CURRENT_USER.locationId}`} />
                ...
                <StatCard label={isAdmin() ? "Total Shops" : "Your Location"} value={isAdmin() ? shops.length : "1"} sub={isAdmin() ? `${DASHBOARD_STATS?.totalSuppliers || 8} vendors` : CURRENT_USER.locationId} />
            </div>
