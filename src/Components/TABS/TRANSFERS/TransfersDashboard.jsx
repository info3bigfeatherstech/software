// TABS/TRANSFERS/TransfersDashboard.jsx
//
// Owns the horizontal tab bar for the Transfers section.
// Reads `ctab` from URL, renders the matching sub-tab component.
// Add new sub-tabs in transfersTabRegistry.js only — nothing here changes.
    ...
    if (filteredTabs.length === 0) {
        return (
            <div className="text-center py-20 text-gray-400 text-sm">
                No transfer tabs available for your role.
            </div>
        );
    }
...
//     const activeCtab = searchParams.get("ctab") || TRANSFERS_TAB_REGISTRY[0]?.id;
//     const activeConfig = TRANSFERS_TAB_REGISTRY.find(t => t.id === activeCtab) || TRANSFERS_TAB_REGISTRY[0];
//     const SubComponent = activeConfig?.component ?? null;
...
//             {/* ── Horizontal tab bar ─────────────────────────────────────────── */}
//             <div className="flex items-center gap-1 border-b border-gray-200 mb-6">
//                 {TRANSFERS_TAB_REGISTRY.map((tab) => {
//                     const isActive = activeCtab === tab.id;
//                     return (
//                         <button
//                             key={tab.id}
