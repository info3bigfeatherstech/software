...
export default function TransferHistoryTab() {
    ...
    // Fetch variant ledger - THIS ONLY FETCHES THE SELECTED VARIANT
    const { data: variantLedger, refetch: refetchVariant, isFetching: isFetchingVariant } = useGetVariantLedgerQuery(
    ...
    const handleViewVariantLedger = (variantId, productName = "") => {
        setSelectedVariantId(variantId);
        setSelectedVariantName(productName);
        setShowVariantModal(true);
        // Refetch will happen automatically when modal opens due to skip condition
    };
    
    // Refetch when modal opens
    useEffect(() => {
        if (showVariantModal && selectedVariantId) {
            refetchVariant();
        }
                            ...
                            {ledger.map((entry) => (
                                <tr key={entry.ledger_id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3 text-center">
                                        <button
                                            onClick={() => handleViewVariantLedger(entry.variant_id, entry.product_id)}
                    ...
                    <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-4 max-h-[80vh] flex flex-col">
                        ...
                        <div className="flex items-center justify-between p-5 border-b border-gray-100">
                            <div>
                                ...
                                <p className="text-xs text-gray-400 font-mono mt-0.5 break-all">{selectedVariantId}</p>
                            </div>
                            <button
                                onClick={() => {
                        ...
                        <div className="flex-1 overflow-y-auto p-5">
                            ...
                            {!isFetchingVariant && (!variantLedger?.ledger || variantLedger.ledger.length === 0) && (
                                <div className="text-center py-12 text-gray-400 text-sm">
                                    No ledger entries found for this variant
                                </div>
                            )}
                            {!isFetchingVariant && variantLedger?.ledger && variantLedger.ledger.length > 0 && (
                                <table className="w-full text-sm">
                                        ...
                                        {variantLedger.ledger.map((entry) => (
                                            <tr key={entry.ledger_id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-3 py-2 text-right font-semibold">
                                                    <span className={entry.quantity > 0 ? "text-green-600" : "text-red-500"}>
                                                        {entry.quantity > 0 ? `+${entry.quantity}` : entry.quantity}
                                                    </span>
                                                  </td>
                                    ...
                                    <tfoot className="bg-gray-50">
                                        <tr>
                                            <td colSpan={5} className="px-3 py-2 text-xs text-gray-500">
                                                Total entries: {variantLedger.ledger.length}
                                            </td>
                                        </tr>
                                    </tfoot>
                                </table>
