...
import { Save, RefreshCw, Store } from "lucide-react";
    ...
    if (isLoading || !formData) {
        return (
            <div className="flex flex-col items-center justify-center py-24 text-gray-400 gap-3">
                ...
                <span className="text-sm font-medium tracking-wide">Loading system master data profile...</span>
            </div>
        );
    }
        ...
        <div className="max-w-4xl mx-auto px-1 py-2">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 pb-5 border-b border-gray-100">
                <div className="flex items-center gap-3.5">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-gray-900 tracking-tight">Shop Profile Configuration</h2>
                        <p className="text-xs text-gray-500 mt-0.5">
                            Modify master operational values, system contact directories, and transactional billing variables.
                        </p>
                    </div>
                </div>
                ...
                <button
                    ...
                    onClick={() => refetch()}
                    disabled={isFetching}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-gray-600 hover:text-gray-900 bg-white border border-gray-200 hover:border-gray-300 rounded-xl shadow-sm transition-all duration-200 disabled:opacity-60"
                    title="Synchronize Data Model"
                >
                    <RefreshCw className={`w-3.5 h-3.5 ${isFetching ? "animate-spin text-blue-500" : ""}`} />
                    {isFetching ? "Syncing..." : "Sync Profile"}
                </button>
            </div>

            {/* Core Card Element */}
            <div className="bg-white border border-gray-200 rounded-2xl p-5 md:p-7 shadow-sm hover:shadow-md/5 transition-all duration-300">
                <ShopProfileFormBody
                    formData={formData}
                ...
                <div className="flex justify-end mt-8 pt-5 border-t border-gray-100">
                    <button
                        ...
                        onClick={handleSave}
                        disabled={isSaving}
                        className="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white text-sm font-semibold rounded-xl transition-all duration-150 shadow-sm shadow-blue-500/10 hover:shadow-blue-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        {isSaving ? (
                            <>
...
// down code is working but upper code have update ui
// import React, { useEffect, useState } from "react";
